...
public class Model {
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        int targetY = y;
        if(targetY<getBoard().size()-1){
            while(tile(x,targetY+1)==null){
                targetY++;
                if(targetY>=getBoard().size()-1){
                    break;
                }
            ...
            if(targetY<getBoard().size()-1){
                if(myValue==tile(x,targetY+1).value()&&tile(x,targetY+1).wasMerged()==false) {
                    targetY++;
